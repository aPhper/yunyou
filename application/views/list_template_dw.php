...
<div class="formbody">
  <div class="formtitle"> <span>配置模板列表 </span> </div>
  <?php if($flagopen === 1){ ?>
  <ul class="yun_bt">
     ...
     <li class="dq"><a href="<?php echo base_url('template/list_page/1') ?>">配置模板信息列表</a></li>
     <li><a href="<?php echo base_url('gateway/list_page/1') ?>">网关列表</a></li>
  </ul>
  <?php }else{ ?>
  <ul class="yun_bt">
     <li class="dq"><a href="<?php echo base_url('template/list_page/2') ?>">配置模板信息列表</a></li>
  </ul>
  <?php } ?>
  <table class="tablelist">
    ...
    <tr>
        ...
        <td><?php echo $value['col_zone_name']?></td>
        <td><?php echo $value['col_offering_name']?></td>
        <td><?php echo $value['col_ostype_name']?></td>
        <td><?php echo ($value['col_valid'] ==='Y' ? '有效':'无效' )?></td>
        <td><?php if($value['col_check'] === 0){
            echo '未审核';
        }elseif ($value['col_check'] === 1){
            echo '审核通过';
        }else{
            echo '审核未通过';
        }
            ?></td>
        <?php if($flagopen === 1){ ?>
        <td><a href="<?php echo base_url('template/select/'.$value['col_id'].'/'.$currPage) ?>" class="tablelink">
                                    修改
          </a>
          <?php if ($value['col_valid'] ==='Y'){ ?>
          &nbsp;&nbsp;&nbsp;
          <a href="javascript:void(0)" class="tablelink" onclick="deleteOne('<?php echo base_url('template/delete/'.$value['col_id'].'/'.$currPage) ?>','<?php echo base_url('template/list_page/1/'.$currPage) ?>')" >
                                     删除
          </a>
          <?php }?>
          </td>
...
<?php }}else{ ?>
    <tr align="center">
        <td colspan="7">暂时没有记录，请先<a href="<?php echo base_url('template') ?>">新建template</a></td>
    </tr>
<?php }?>
    </tbody>
  </table>
