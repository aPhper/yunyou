...
<div class="formbody">
  ...
  <ul class="yun_bt">
     ...
     <li class="dq"><a href="<?php echo base_url('diskoffering/list_page/1') ?>">Diskoffering列表</a></li>
  ...
  <table class="tablelist">
    ...
    <tr>
        ...
        <td><?php echo $value['col_size']?></td>
        <td><?php echo $value['col_storagetype']?></td>
        <td><?php echo ($value['col_valid'] ==='Y' ? '有效':'无效' )?></td>
        <?php if($flagopen === 1){ ?>
        <td><a href="<?php echo base_url('diskoffering/select/'.$value['col_id'].'/'.$currPage) ?>" class="tablelink">
                                    修改
          </a>
          <?php if ($value['col_valid'] ==='Y'){ ?>
          &nbsp;&nbsp;&nbsp;
          <a href="javascript:void(0)" class="tablelink" onclick="deleteOne('<?php echo base_url('diskoffering/delete/'.$value['col_id'].'/'.$currPage) ?>','<?php echo base_url('diskoffering/list_page/1/'.$currPage) ?>')" >
                                     删除
          </a>
          <?php }?>
          </td>
...
<?php }}else{ ?>
    <tr align="center">
        <td colspan="6">暂时没有记录，请先<a href="<?php echo base_url('diskoffering') ?>">新建diskoffering</a></td>
    </tr>
<?php }?>
    </tbody>
  </table>
