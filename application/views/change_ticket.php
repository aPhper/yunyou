...
<div class="formbody">
  <div class="formtitle"><span>转单</span></div>
  <?php echo form_open('manage_ticket/change_ticket')?>
  ...
  <ul class="forminfo">
   <li>
          <label>转给</label>
          <div class="vocation">
            <select name='username' class="select1">
            <?php foreach($user_list as $key =>$value){?>
              <option value='<?php echo $value['col_id']?>'><?php echo $value['col_nickname'];?></option>
              <?php }?>
            </select>
          </div>
    </li>
    <li>
      <label>&nbsp;</label>
      <input name="submit" type="submit" class="btn click" value="转单"/>
    </li>
  </ul>
  </form>
</div>
