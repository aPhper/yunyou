...
function handle(col_zone_id,col_zone_name){
	window.opener.document.getElementsByName("col_zone_name")[0].value = col_zone_name;
	window.opener.document.getElementsByName("col_zone_id")[0].value = col_zone_id;
	window.close();
}
</script>
</head>
...
<div class="formbody">
  <div class="formtitle"> <span>diskoffering列表 </span> </div>
  <?php if($flagopen === 1){ ?>
  <ul class="yun_bt">
     ...
     <li class="dq"><a href="<?php echo base_url('gateway/list_page/1') ?>">网关列表</a></li>
  </ul>
  <?php }else{ ?>
  <ul class="yun_bt">
     <li class="dq"><a href="<?php echo base_url('gateway/list_page/1') ?>">网关列表</a></li>
  ...
  <table class="tablelist">
    ...
    <tr>
        ...
        <td><?php echo $value['col_port']?></td>
        <td><?php echo $value['col_url']?></td>
        <td><?php echo $value['col_user']?></td>
        <td><?php echo $value['col_passwd']?></td>
        <td><?php echo ($value['status'] ==='Y' ? '有效':'无效' )?></td>
        <?php if($flagopen === 1){ ?>
        <td><a href="<?php echo base_url('gateway/select/'.$value['col_id'].'/'.$currPage) ?>" class="tablelink">
                                    修改
          </a>
          <?php if ($value['status'] ==='Y'){ ?>
          &nbsp;&nbsp;&nbsp;
          <a href="javascript:void(0)" class="tablelink" onclick="deleteOne('<?php echo base_url('gateway/delete/'.$value['col_id'].'/'.$currPage) ?>','<?php echo base_url('gateway/list_page/1/'.$currPage) ?>')" >
                                     删除
          </a>
          <?php }?>
          </td>
...
<?php }}else{ ?>
    <tr align="center">
        <td colspan="8">暂时没有记录，请先<a href="<?php echo base_url('gateway') ?>">新建gateway</a></td>
    </tr>
<?php }?>
    </tbody>
  </table>
