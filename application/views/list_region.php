...
function handle(col_region_id,col_region_name){
	window.opener.document.getElementsByName("col_region_name")[0].value = col_region_name;
	window.opener.document.getElementsByName("col_region_id")[0].value = col_region_id;
	window.close();
}
...
<div class="formbody">
  <div class="formtitle"> <span> region列表 </span> </div>
  <?php if($flagopen === 1){ ?>
  <ul class="yun_bt">
     <li><a href="<?php echo base_url('cloud/list_page/1') ?>">云平台信息列表</a></li>
     <li class="dq"><a href="<?php echo base_url('region/list_page/1') ?>">region列表</a></li>
  ...
  <?php }else{ ?>
  <ul class="yun_bt">
     <li class="dq"><a href="<?php echo base_url('region/list_page/2') ?>">region列表</a></li>
  </ul>
  <?php } ?>
  <table class="tablelist">
    ...
    <tr>
        ...
        <td><a href="<?php echo base_url('ostype/select/'.$value['col_id'].'/'.$currPage) ?>" class="tablelink">
          ...
          &nbsp;&nbsp;&nbsp;
          <a href="javascript:void(0)" class="tablelink" onclick="deleteOne('<?php echo base_url('region/delete/'.$value['col_id'].'/'.$currPage) ?>','<?php echo base_url('region/list_page/1/'.$currPage) ?>')" >
                                     删除
          </a>
          <?php }?>
          </td>
...
<?php }}else{ ?>
    <tr align="center">
        <td colspan="5">暂时没有记录，请先<a href="<?php echo base_url('region') ?>">新建region</a></td>
    </tr>
<?php }?>
    </tbody>
  </table>
