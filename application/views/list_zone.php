...
<div class="formbody">
  ...
  <?php }else{ ?>
  <ul class="yun_bt">
     <li class="dq"><a href="<?php echo base_url('zone/list_page/2') ?>">zone列表</a></li>
  </ul>
  <?php } ?>
  <table class="tablelist">
    ...
    <tr>
        ...
        <td><?php echo $value['col_desc']?></td>
        <td><?php echo ($value['col_valid'] ==='Y' ? '有效':'无效' )?></td>
        <?php if($flagopen === 1){ ?>
        <td><a href="<?php echo base_url('zone/select/'.$value['col_id'].'/'.$currPage) ?>" class="tablelink">
                                    修改
          </a>
          <?php if ($value['col_valid'] ==='Y'){ ?>
          &nbsp;&nbsp;&nbsp;
          <a href="javascript:void(0)" class="tablelink" onclick="deleteOne('<?php echo base_url('zone/delete/'.$value['col_id'].'/'.$currPage) ?>','<?php echo base_url('zone/list_page/1/'.$currPage) ?>')" >
                                     删除
          </a>
          <?php }?>
          </td>
...
<?php }}else{ ?>
    <tr align="center">
        <td colspan="5">暂时没有记录，请先<a href="<?php echo base_url('zone') ?>">新建zone</a></td>
    </tr>
<?php }?>
    </tbody>
  </table>
